...
| `cpa_auth_dir` | 额外 CPA 热加载目录；留空时仍写入本次 `output/runs/<run_id>/cpa/` |
| `cpa_mirror_dir` | 可选第二本地镜像目录（再写一份 `xai-*.json`） |
| `cpa_remote_url` | 远程 CPA 地址，如 `http://你的CPA地址:8317` |
| `cpa_management_key` | 远程 CPA 管理密钥（`remote-management.secret-key` 明文） |
| `output_root` | 运行输出根目录，默认 `output/runs` |
| `output_run_id` | 固定本次 run 文件夹名；留空则自动 `时间戳__驱动__求解__代理` |
| `output_run_suffix` | 自动 run_id 后附加标签（多进程可用 `w1` / `w2`） |
...
| `turnstile_warmup_seconds` | 填完资料后预热秒数（默认 `4`），给 iframe / once-click 扩展时间 |
| `turnstile_settle_seconds` | 填完后**被动**观察 soft-pass 的秒数（默认 `12`），期间不点验证框 |
| `turnstile_extension_grace_seconds` | 本地点击路径里先等扩展 once-click 的秒数（默认 `5`） |
| `turnstile_local_max_clicks` | 扩展后 Python CDP 最多点几次（默认 `2`；狂点会重置 managed CF） |
| `turnstile_local_click_interval` | 本地点击最小间隔秒（默认 `8`） |
| `turnstile_local_timeout` | 无 CapMonster 时本地静默求解总预算秒（默认 `50`） |
| `turnstile_fill_loop_extra_click` | 填表循环里是否额外 CDP 点验证（默认 `false`，避免 spam） |
...
# 指定临时邮箱域名 + run 后缀（并行双进程时各自不同域名/后缀，共用一份 config.json）
python grok_register_ttk.py cli -y --driver browser_use --country jp \
  --domain flyovernow.ccwu.cc --run-suffix flyover \
  --count 15 --success-target 10

# 可选：整份配置文件（多数场景用 --domain 即可）
# python grok_register_ttk.py cli --config /path/to/config.other.json ...
...
- 关 CapMonster 时走**静默本地路径**：扩展 once-click → 被动等待 soft-pass → 最多 1–2 次稀疏 CDP（见 `turnstile_local_*`）。**不要狂点**——live 观察：spam 更容易 “Pengesahan gagal”，停点后 idle 反而有时 soft-pass
- managed CF **仍强烈依赖出口 IP**；脏 IP 上静默也救不了，请开 CapMonster 或换干净代理
...
3. 本地：始终写入本次 `output/runs/<run_id>/cpa/xai-<email>.json`；若配置了 `cpa_auth_dir` / `cpa_mirror_dir` 再镜像一份（CPA 热加载）
...
`cpa_auth_dir` 填 CPA 实际监听的 auth 目录路径即可；不填也会在 `output/runs/<run_id>/cpa/` 留下本次 run 的 auth。
...
每次注册任务（GUI「开始」或 CLI 一轮）会在标准目录下创建**一个 run 文件夹**：
...
output/
  ...
  runs/
    .gitkeep
    latest -> <run_id>/    # 软链接，指向最近一次
    20260715_001530__browser_use__capmonster__proxy-jp/
      run.json             # 元数据（驱动 / 求解 / 代理摘要，无密钥）
      accounts.sso.txt     # email----password----sso
      accounts.jsonl       # 同上 + 时间戳 / 驱动标签
      mail_credentials.txt # 临时邮箱 jwt
      register.log         # 本 run 控制台日志镜像
      failed.txt           # 失败槽位摘要
      summary.json         # 结束后成功/失败/耗时
      cpa/                 # cpa_auto_add 时的 xai-*.json
```

文件夹名编码：`YYYYMMDD_HHMMSS__<driver>__<solver>__<proxy-tag>[__suffix]`  
例：`browser_use` + CapMonster + Browser Use JP 代理 → `...__browser_use__capmonster__proxy-jp`。

| 路径 | 内容 |
| --- | --- |
| `output/runs/<run_id>/accounts.sso.txt` | 邮箱、密码、SSO（实时追加） |
| `output/runs/<run_id>/accounts.jsonl` | 结构化成功记录 |
| `output/runs/<run_id>/mail_credentials.txt` | 临时邮箱凭证 |
| `output/runs/<run_id>/cpa/` | 本 run 的 CPA auth（`cpa_auto_add`） |
| `output/runs/<run_id>/summary.json` | 成功/失败统计 |
| `output/runs/latest` | 指向最近 run 的 symlink |

仓库只提交 `output/.gitkeep` 与 `output/runs/.gitkeep`；`output/runs/*/` 下的产物与 `config.json` 均在 `.gitignore` 中。请用 `config.example.json` 复制配置。
...
├── run_output.py             # 标准 run 目录 begin/append/end
├── sso_to_auth_json.py       # SSO → CPA 转换（可独立运行）
├── browser_use_adapter.py    # Browser Use Playwright CDP
├── roxy_adapter.py           # RoxyBrowser 附着
├── cf_mail_debug.py          # Cloudflare 邮箱调试
├── config.example.json
├── requirements.txt
├── output/                   # 标准输出根（.gitkeep 入库）
│   └── runs/                 # 每次任务一个子目录
